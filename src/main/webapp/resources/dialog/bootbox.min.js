!function (t, e) {
    ...
    var r, n, i, l, d = {}, s = (d.VERSION = "5.5.3", {en: {OK: "OK", CANCEL: "Cancel", CONFIRM: "OK"}}), b = {
    ...
    function m(t, e, o, a) {
        a && a[0] && (r = a[0].locale || f.locale, (a[0].swapButtonOrder || f.swapButtonOrder) && (e = e.reverse()));
        var r, n, i, t = {
        ...
        return r = c(t, a, o), i = {}, w(n = e, function (t, e) {
            i[e] = !0
        }), w(r.buttons, function (t) {
            if (i[t] === u) throw new Error('button key "' + t + '" is not allowed (options are ' + n.join(" ") + ")")
        }), r
    }
    ...
    return d.locales = function (t) {
        ...
        }), s[t] = {OK: o.OK, CANCEL: o.CANCEL, CONFIRM: o.CONFIRM}, d
        ...
        var t = {};
        return 2 === arguments.length ? t[arguments[0]] = arguments[1] : t = arguments[0], p.extend(f, t), d
    }, d.hideAll = function () {
        ...
        t = function (a) {
            ...
            (a = p.extend({}, f, a)).backdrop ? a.backdrop = "string" != typeof a.backdrop || "static" !== a.backdrop.toLowerCase() || "static" : a.backdrop = !1 !== a.backdrop && 0 !== a.backdrop && "static";
            a.buttons || (a.buttons = {});
            return r = a.buttons, n = h(r), w(r, function (t, e, o) {
                if (p.isFunction(e) && (e = r[t] = {callback: e}), "object" !== p.type(e)) throw new Error('button with key "' + t + '" must be an object');
                e.label || (e.label = t), e.className || (t = !1, t = a.swapButtonOrder ? 0 === o : o === n - 1, e.className = n <= 2 && t ? "btn-primary" : "btn-secondary btn-default")
            }), a
        }(t), p.fn.modal.Constructor.VERSION ? (t.fullBootstrapVersion = p.fn.modal.Constructor.VERSION, a = t.fullBootstrapVersion.indexOf("."), t.bootstrap = t.fullBootstrapVersion.substring(0, a)) : (t.bootstrap = "2", t.fullBootstrapVersion = "2.3.2", console.warn("Bootbox will *mostly* work with Bootstrap 2, but we do not officially support it. Please upgrade, if possible."));
        var o = p(b.dialog), e = o.find(".modal-dialog"), a = o.find(".modal-body"), r = p(b.header), n = p(b.footer),
        ...
        return !0 === t.backdrop && o.on("click.dismiss.bs.modal", function (t) {
            ...
            keyboard: !1,
            show: !1
        }), t.show && o.modal("show", t.relatedTarget), o
    }, d.alert = function () {
        var t = m("alert", ["ok"], ["message", "callback"], arguments);
        if (t.callback && !p.isFunction(t.callback)) throw new Error('alert requires the "callback" property to be a function when provided');
        ...
        if (p.isFunction(t.callback)) return t.buttons.cancel.callback = t.onEscape = function () {
        ...
        var e, t, o, a = p(b.form), r = m("prompt", ["cancel", "confirm"], ["title", "callback"], arguments);
        if (r.value || (r.value = f.value), r.inputType || (r.inputType = f.inputType), t = (r.show === u ? f : r).show, r.show = !1, r.buttons.cancel.callback = r.onEscape = function () {
            return r.callback.call(this, null)
        }, r.buttons.confirm.callback = function () {
            var t;
            if ("checkbox" === r.inputType) t = s.find("input:checked").map(function () {
                return p(this).val()
            }).get(); else if ("radio" === r.inputType) t = s.find("input:checked").val(); else {
                if (s[0].checkValidity && !s[0].checkValidity()) return !1;
                t = "select" === r.inputType && !0 === r.multiple ? s.find("option:selected").map(function () {
                    return p(this).val()
                }).get() : s.val()
            }
        ...
        if (!b.inputs[r.inputType]) throw new Error("Invalid prompt type");
        switch (s = p(b.inputs[r.inputType]), r.inputType) {
            case"text":
            case"textarea":
            case"email":
            case"password":
                s.val(r.value), r.placeholder && s.attr("placeholder", r.placeholder), r.pattern && s.attr("pattern", r.pattern), r.maxlength && s.attr("maxlength", r.maxlength), r.required && s.prop({required: !0}), r.rows && !isNaN(parseInt(r.rows)) && "textarea" === r.inputType && s.attr({rows: r.rows});
                break;
            case"date":
            case"time":
            case"number":
            case"range":
                if (s.val(r.value), r.placeholder && s.attr("placeholder", r.placeholder), r.pattern && s.attr("pattern", r.pattern), r.required && s.prop({required: !0}), "date" !== r.inputType && r.step) {
                ...
                !function (t, e, o) {
                    var a = !1, r = !0, n = !0;
                    if ("date" === t) e === u || (r = E(e)) ? o === u || (n = E(o)) || console.warn('Browsers which natively support the "date" input type expect date values to be of the form "YYYY-MM-DD" (see ISO-8601 https://www.iso.org/iso-8601-date-and-time-format.html). Bootbox does not enforce this rule, but your max value may not be enforced by this browser.') : console.warn('Browsers which natively support the "date" input type expect date values to be of the form "YYYY-MM-DD" (see ISO-8601 https://www.iso.org/iso-8601-date-and-time-format.html). Bootbox does not enforce this rule, but your min value may not be enforced by this browser.'); else if ("time" === t) {
                        if (e !== u && !(r = k(e))) throw new Error('"min" is not a valid time. See https://www.w3.org/TR/2012/WD-html-markup-20120315/datatypes.html#form.data.time for more information.');
                        if (o !== u && !(n = k(o))) throw new Error('"max" is not a valid time. See https://www.w3.org/TR/2012/WD-html-markup-20120315/datatypes.html#form.data.time for more information.')
                    } else {
                    ...
                    return a
                }(r.inputType, r.min, r.max) || (r.min !== u && s.attr("min", r.min), r.max !== u && s.attr("max", r.max));
                break;
            case"select":
                var n = {}, i = r.inputOptions || [];
                if (!p.isArray(i)) throw new Error("Please pass an array of input options");
                if (!i.length) throw new Error('prompt with "inputType" set to "select" requires at least one option');
                r.placeholder && s.attr("placeholder", r.placeholder), r.required && s.prop({required: !0}), r.multiple && s.prop({multiple: !0}), w(i, function (t, e) {
                    var o = s;
                    if (e.value === u || e.text === u) throw new Error('each option needs a "value" property and a "text" property');
                    e.group && (n[e.group] || (n[e.group] = p("<optgroup />").attr("label", e.group)), o = n[e.group]);
                    var a = p(b.option);
            ...
            case"checkbox":
                ...
                if (!(i = r.inputOptions || []).length) throw new Error('prompt with "inputType" set to "checkbox" requires at least one option');
                s = p('<div class="bootbox-checkbox-list"></div>'), w(i, function (t, o) {
                    if (o.value === u || o.text === u) throw new Error('each option needs a "value" property and a "text" property');
                    var a = p(b.inputs[r.inputType]);
            ...
            case"radio":
                ...
                if (!(i = r.inputOptions || []).length) throw new Error('prompt with "inputType" set to "radio" requires at least one option');
                var s = p('<div class="bootbox-radiobutton-list"></div>'), c = !0;
                w(i, function (t, e) {
                    if (e.value === u || e.text === u) throw new Error('each option needs a "value" property and a "text" property');
                    var o = p(b.inputs[r.inputType]);
                    o.find("input").attr("value", e.value), o.find("label").append("\n" + e.text), r.value !== u && e.value === r.value && (o.find("input").prop("checked", !0), c = !1), s.append(o)
                }), c && s.find('input[type="radio"]').first().prop("checked", !0)
        }
        return a.append(s), a.on("submit", function (t) {
            t.preventDefault(), t.stopPropagation(), e.find(".bootbox-accept").trigger("click")
        }), "" !== p.trim(r.message) && (o = p(b.promptMessage).html(r.message), a.prepend(o)), r.message = a, (e = d.dialog(r)).off("shown.bs.modal", g), e.on("shown.bs.modal", function () {
            s.focus()
        }), !0 === t && e.modal("show"), e
    }, d
});